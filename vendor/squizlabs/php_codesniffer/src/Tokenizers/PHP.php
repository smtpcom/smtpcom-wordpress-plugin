...
class PHP extends Tokenizer
{
                ...
                */
                ...
                if ($parenthesisCloser !== false) {
                    ...
                    if (
                        ...
                        for ($x; $x < $numTokens; $x++) {
                            if (
                                (is_array($tokens[$x]) === false && $tokens[$x] !== '|')
                                || (is_array($tokens[$x]) === true && isset($allowed[$tokens[$x][0]]) === false)
                            ) {
                                break;
                            } elseif (is_array($tokens[$x]) === true && $tokens[$x][0] === T_ARRAY) {
                                $tokens[$x][0] = T_STRING;
            ...
            if ($this->tokens[$i]['code'] === T_FUNCTION) {
                ...
                */
                ...
                if (
                    ...
                    if (
                        isset($this->tokens[$last]['parenthesis_owner']) === true
                        ...
                        if (isset($this->tokens[$last]['parenthesis_opener']) === true) {
                            ...
                            if ($this->tokens[$x]['code'] === T_FN) {
                                for (--$x; $x >= 0; $x--) {
                                    if (
                                        isset(Util\Tokens::$emptyTokens[$this->tokens[$x]['code']]) === true
                                        || $this->tokens[$x]['code'] === T_BITWISE_AND
                                    ) {
                                        continue;
                                    }
    ...
    private function findCloser(array &$tokens, $start, $openerTokens, $closerChar)
    {
        ...
        for ($x = $start; $x < $numTokens; $x++) {
            if (
                in_array($tokens[$x], $openerTokens, true) === true
    ...
    private function createAttributesNestingMap()
    {
        ...
        for ($i = 0; $i < $this->numTokens; $i++) {
            if (
                isset($this->tokens[$i]['attribute_opener']) === true
                ...
                isset($this->tokens[$i]['attribute_closer']) === true
