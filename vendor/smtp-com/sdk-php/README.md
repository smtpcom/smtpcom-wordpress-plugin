...
``` bash
$ composer require smtp-com/sdk-php
```
...
$parameters = [
    'first_name'      => 'string', // First name of account owner
    'last_name'       => 'string', // Last name of account owner
    'email'           => 'string', // Email address of account owner
    'company_name'    => 'string', // Account owner’s company name
    'phone'           => 'string', // Phone number of account owner
    'website'         => 'string', // Website of account owner
    'address.street'  => 'string', // Account owner’s street address
    'address.city'    => 'string', // Account owner’s city
    'address.state'   => 'string', // Account owner’s state
    'address.country' => 'string'  // Account owner’s country
];
$ps->account()->update($parameters);
```
...
$ps->keys($apiKey)->update($parameters);
```
...
```php
$ps->reports()->periodic($reportId)->delete();
```
...
$parameters = [
    'frequency'   => 'string',  // *Required. Report frequency
    'report_time' => 'integer', // *Required. The hour at which the report should be sent.
    'channel'     => 'string',  // Name of channel (sender). If not specified all channels will be reported.
    'events'      => 'string'   // Filter by event type.
];
$ps->reports()->periodic($reportId)->update($parameters);
```
